...
export const LOWER_NOTE = C_4_MIDI
export const HIGHER_NOTE = B_5_MIDI
export const SEMI_TONES_IN_OCTAVE = 12
...
export function fromMidi(midi: MidiValue): Note {
  const pianoRange = midi - C_1_MIDI
  const octave = (Math.floor(pianoRange / SEMI_TONES_IN_OCTAVE) + 1) as OctaveIndex

  const index = pianoRange % SEMI_TONES_IN_OCTAVE
...
export function generateNotes({ fromNote = LOWER_NOTE, toNote = HIGHER_NOTE }: NotesGeneratorProps = {}): Note[] {
